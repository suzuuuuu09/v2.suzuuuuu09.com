			...
			(node: Link, index: number | undefined, parent: any) => {
				...
				switch (urlType) {
					...
					case "twitter": {
						...
						if (tweetId) {
							...
							const promise = (async () => {
								...
								const htmlValue = createGridCardHtml(
									url,
									title,
									site,
									"twitter",
					...
					case "google-slides": {
						...
						if (embedUrl) {
							const htmlValue = createGoogleSlidesEmbedHtml(
								embedUrl,
								linkTitle,
							);
							parent.children[index] = {
					...
					case "other": {
						...
						const promise = (async () => {
							...
							htmlValue = createStandardCardHtml(
								url,
								title,
								site,
								siteClass,
	...
	try {
		...
		if (
			...
			if (
				...
				urlObj.pathname.includes("/pubembed")
			) {
				return url;
			}
